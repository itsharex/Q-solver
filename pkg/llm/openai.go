...
// OpenAIProvider 实现了 Provider 接口
...
func (p *OpenAIProvider) GenerateContentStream(ctx context.Context, history []openai.ChatCompletionMessageParamUnion, onToken func(string)) (string, error) {
	...
	if err := stream.Err(); err != nil {
		...
		//从json前边开始扫第一个冒号出现，要不然直接就扫到json里面的冒号了，直接出问题了
...
// 转录音频
func (p *OpenAIProvider) Transcription(ctx context.Context, audioByte []byte) (string, error) {
	params := openai.AudioTranscriptionNewParams{
		Model:    "whisper-large-v3",
		File:     openai.File(bytes.NewReader(audioByte), "voice.wav", "audio/wav"),
		Language: openai.String("zh"),
	}
...
// GetBalance 验证 API Key 是否有效
func (p *OpenAIProvider) GetBalance(ctx context.Context) (float64, error) {
	// 使用 List Models 接口验证 Key 是否有效
	_, err := p.client.Models.List(ctx)
	if err != nil {
		return 0, err
	}
	// 验证成功返回 0，前端会根据 nil error 判断为连接成功
	return 0, nil
}
